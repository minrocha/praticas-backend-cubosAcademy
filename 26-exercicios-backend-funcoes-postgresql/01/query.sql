...
select sum(estoque) as "Quantidade De Medicamentos" from farmacia;
...
sum(estoque) as "Quantidade Produtos" 
from farmacia 
...
select concat(medicamento, ' ', '(', categoria, ')') as "Nome e Categoria" 
from farmacia 
where categoria is not null;
...
from usuarios 
where cast(cadastro as date) >= cast('2018-01-01' as date)
AND cast(cadastro as date) <= cast('2018-12-31' as date);
